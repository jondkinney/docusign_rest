...
module DocusignRest
  ...
  class Client
    ...
    def get_composite_template(server_templates, signers)
      composite_array = []
      server_templates.each_with_index  do |template, index|
        server_template_hash = Hash[:sequence, index+1, \
          :templateId, template[:server_template_id]]
    ...
    # headers               - Optional hash of headers to merge into the existing
    #                         required headers for a multipart request.
    # server_templates      - Array of templates uploaded to DocuSign. Templates
    #                         will be added in the order they appear in the array.
    #                         Templates can contain 'document' parameters for uploading
    #                         pdf files. The parameters will be used to add the file
    #                         to a multipart post request only if 'document_id' is found.
    #                         Parameters expected when 'document_id' is found:
    #                           document_id - The unique Id for the document in this specific envelope.
    #                           path - The local path to the document file
    #                           name - The name of the document. This can be a maximum of 100 characters.
    #
    # Returns a JSON parsed response body containing the envelope's:
    ...
    def create_envelope_from_composite_template(options={})
      files = options[:server_templates].select{|template| template[:document_id]}
      files.map! do |template|
        {path: template[:path], name: template[:name]}
      end
      ios = create_file_ios(files)
      file_params = create_file_params(ios)

      post_body = {
        status:             options[:status],
        compositeTemplates: get_composite_template(options[:server_templates], options[:signers])
      }.to_json
      ...
      request = if files.empty?
                  ...
                  initialize_net_http_multipart_post_request(
                    uri, post_body, file_params, headers(options[:headers])
                  )
                end
